...
contract('Flight Surety Tests', async (accounts) => {
  ...
  it('(airline) cannot register an Airline using registerAirline() if it is not funded 4', async () => {
    ...
    try {
      await config.flightSuretyApp.registerAirline(newAirline2, "Indian Air Lines", { from: config.firstAirline });
  ...
  it('(airline) Registration of fifth and subsequent airlines requires multi-party consensus of 50% of registered airlines, 7 ', async () => {
    ...
    try {
      ...
      await config.flightSuretyApp.registerAirline(newAirline3, "Indian Air Lines3", { from: newAirline2 });
      await config.flightSuretyApp.registerAirline(newAirline4, "Indian Air Lines4", { from: config.firstAirline });
      ...
      await config.flightSuretyApp.registerAirline(newAirline5, "Indian Air Lines5", { from: newAirline2 });

      await config.flightSuretyApp.registerAirline(newAirline5, "Indian Air Lines5", { from: newAirline3 });

      await config.flightSuretyApp.registerAirline(newAirline5, "Indian Air Lines5", { from: newAirline4 });
